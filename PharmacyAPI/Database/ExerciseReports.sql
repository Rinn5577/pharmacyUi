...
SELECT DrugName, UnitCount, UnitPrice, TotalPrice, DeliveryDate, Pharmacies.Name AS PharmacyName, Warehouse.Name AS WarehouseName
FROM Delivery
INNER JOIN Pharmacies ON Delivery.PharmacyId = Pharmacies.Id
...
SELECT CONCAT(FirstName, ' ', LastName) AS Name, Pharmacies.Name AS PharmacyName, PrimaryDrugSold, 
...
FROM Pharmacist
INNER JOIN Pharmacies ON PharmacyId = Pharmacies.Id
ORDER BY Name;
